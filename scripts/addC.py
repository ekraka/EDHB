...
def func(filename,d):
        ...
        for line in lines[index:]:
                ...
                if ref==2:
                        #print line
                        lis+=[line.strip().split()[-5]]
                if ref==1 and '--------------------------------------------------------------' in line:
                        ref=2
...
def get_ids(path,suffix,d):
        ...
        for line in lines:
                ...
                if len(line.strip().split())==0 and ref>0:
                        ref+=1
                if ref==2:
                        lines[i-1]=' '.join(lines[i-1].strip().split())+',  \n'
                if ref==2 or ref==3:
                        ref+=1
                if ref==4:
...
def addC(path,p_id):
        ...
        for line in lines:
                ...
                if len(line.strip().split())==0 and ref>0:
                        ref+=1
                if ref==2:
                        lines[i-1]=' '.join(lines[i-1].strip().split())+', AcceptorChargeNBO, H_ChargeNBO \n'
                if ref==2 or ref==3:
                        ref+=1
                if ref==4:
                        if i2==0:
                        ...
                        try:
                                ...
                                lm[str(i2+1)+'.']="{:>10} {:>10}".format(*p_id[str(i2+1)+'.'])
                        except ValueError:
                                lm[str(i2+1)+'.']='None'
                        #lines[i]=lines[i].strip()+' '+lmodes[i2]+'\n'
                        i2+=1
...
def job(path, filename): # give fchk 
        ref=0
        d={}
        n=0
